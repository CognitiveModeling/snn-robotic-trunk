#ifndef __AMSGRAD_OPTIMIZER_H__
#define __AMSGRAD_OPTIMIZER_H__
...
namespace SNN {
    ...
    namespace Optimizers {
        ...
        class AMSGradOptimizer: public Interfaces::BasicOptimizer {
            ...
            public:
                ...
                virtual FloatType calcWeightChange() {
                    ...
                    FloatType unbiased_v = (v / (1 - beta2Product)) * beta2Product + vMax * (1 - beta2Product);

                    signOscillationMagnitude = 0.9 * signOscillationMagnitude + 
                                               (1.0 - 0.9) * sgn(gradient);

                    FloatType unbiased_s = signOscillationMagnitude / (1 - beta1Product);
                    updateCounter++;
                    if (updateCounter % decayIntervall == 0)
                        learnRate *= decayFactor;

                    return -1.0 * pow(unbiased_s, 2) * learnRate * unbiased_m / (sqrt(unbiased_v) + epsilon) - 
                ...
                AMSGradOptimizer(
                    ...
                    beta1Product(1),
                    beta2Product(1),
                    weightDecay(weightDecay) { doReset(); }
            ...
            /* returns the learn rate of this */
            FloatType getLearnRate() {
                return learnRate;
            }
#endif
